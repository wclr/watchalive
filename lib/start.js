...
var statServer = function(options){
                ...
                setTimeout(function(){
                    ...
                    if (changed.indexOf(filepath) === -1){
                        ...
                        changeTimeout = setTimeout(function(){
                            ...
                            var allowedFiles = changed.filter(function(file){
                                return file.match(/\.(css|less)$/)
                            })

                            if (allowedFiles.length !== changed.length){
                                ...
                                async.map(filesToSend, function(filename, cb){
                                    ...
                                    fs.readFile(filename, 'utf8', function(err, data){
                                        if (err){
                                            console.log('Error reading file'.error, filename)
                                            cb(err)
                                        } else {

                                            if (lessCssMap[filename]){
                                                parseLess(filename, data, function(err, cssData){
                                                    if (err) return cb(err)
                                                    cb(null, {file: getServedFileName(lessCssMap[filename]), data: cssData})
                                                })
                                            } else {
                                                cb(null, {file: getServedFileName(filename), data: data})
                                            }
                                        }
    ...
    app.get(/.css$/, function(req, res, next){
        ...
        //console.log('Request css', req.url, lessFilePath)
        ...
        config.routes.forEach(function(route) {
            ...
            var createRoute = function(url, target){
                ...
                app.get(url, function (req, res, next) {
                    ...
                    req.url = target.replace(/:[\w\d]+/ig, function(param){return req.params[param.slice(1)] || param})

                    //console.log('route get after replace', req.url)
