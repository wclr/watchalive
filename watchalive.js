...
var config = {
    port: 7000,
    base: cwd,
    configFileName: 'watchalive.json',
    clientLibName: 'watchalive.js'
}

if (fs.existsSync(cwd + '/' + config.configFileName)){
    try {
        var fileConfig = JSON.parse(fs.readFileSync(cwd + '/' + config.configFileName))
        for (var prop in fileConfig){
            config[prop] = fileConfig[prop]
        }
        if (fileConfig.base){
            config.base = path.resolve(cwd, fileConfig.base)
        }
        //console.log('watchalive config:', config)
    } catch(e){
